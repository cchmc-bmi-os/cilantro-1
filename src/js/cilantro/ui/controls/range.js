...
define([
    ...
    var RangeControl = base.Control.extend({
        ...
        events: {
            ...
            'click .range-clear-button': 'resetRange',
	...
	resetRange: function(event) {
	    ...
	    this.ui.range.trigger('range:updated');

        },

	/* 
	 * Update range values based on data sent by click on graph event
	 */
	chartClick: function(event, options) {
	    var updated_value = options.bar_x;
	    var recipient;
	    var value = [this.ui.lowerBound[0].value, this.ui.upperBound[0].value];

	    if( this.ui.lowerBound.is(":focus") ) {
	    	recipient = 0;
		this.ui.lowerBound[0].value = updated_value;
	    } else if( this.ui.upperBound.is(":focus") ) {
	    	recipient = 1;
		this.ui.upperBound[0].value = updated_value;
	    } else if ( this.ui.lowerBound[0].value == "" ) {
	    	recipient = 0;
	    } else if ( this.ui.upperBound[0].value == "" ) {
	    	recipient = 1;
	    } else {
	    	recipient = 1;
	    }

	    value[recipient] = updated_value; 
