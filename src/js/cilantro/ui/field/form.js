...
define([
    ...
    var FieldForm = Marionette.Layout.extend({
	    ...
	    'chart:click @ui.chart': 'chartClick',
	    'chart:refresh_selected @ui.chart': 'rangeUpdate',
	    'range:updated @ui.range': 'rangeUpdate'
        },
	...
	/* 
	 * The range control or the graph can trigger this event to update
	 * the selected bars based on the range values in the
	 * interface (text boxes)
	 */
	rangeUpdate: function(event) {
	    // get the controls values
	    options = {}
	    $low = $( this.ui.range_lower.selector );
	    $high = $( this.ui.range_upper.selector );
	    values = [undefined, undefined];

	    // If the graph is used with a range control
	    if( $low.length != 0 ) {
		values[0] = $low[0].value
	    }

	    if( $high.length != 0 ) {
		values[1] = $high[0].value
	    }

	    options['values'] = values;
	    $( this.ui.chart.selector ).trigger("form:refresh_selected", options);
	},

	chartClick: function(event, options) {
	    $( this.ui.range.selector ).trigger('chart:click', options); // sent to range
	},
